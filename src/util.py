...
def re_shuffle(src_path, des_path, test = 0.2,train = 0.6):
    """
    src_path: source directory of data
    des_path: destination of shuffled data
    test:   ratio of test file
    train:  ratio of train file
    """
    if src_path == des_path:
        raise Exception("src path and des path cannot be the same")

    partitions = {"/test":test,"/train":train,"/valid":1 - test - train}
    allfiles = [ src_path + suffix + "/" +f for suffix in partitions for f in listdir(src_path + suffix) if f.endswith(".mid") ]
    print len(allfiles)
    deses = [ des_path + suffix for suffix in partitions]
    for des in deses:
        if os.path.isdir(des):
            shutil.rmtree(des)
        os.makedirs(des+ "/")
    counter = Counter(partitions.keys())
    total = 0
    for partition in partitions:
        partitions[partition] = int(len(allfiles)*partitions[partition])
        total += partitions[partition]
    dif = len(allfiles) - total
    partitions["/test"] += dif
    shuffle(allfiles)
    start = 0
    print partitions
    for partition in partitions:
        des = des_path + partition + "/"
        l = partitions[partition]
        for f in allfiles[start:start+l]:
            shutil.copy2(f,des)
        start += l
