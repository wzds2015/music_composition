...
class Model(object):
    ...
    def __init__(self, config, training=False):
        ...
        def create_cell(input_size):
            ...
            if cell_type == "gru" or cell_type== "vanilla":
                cell = cell_class(hidden_size, input_size = input_size)
            else: # cell_type == "lstm":
                cell = cell_class(hidden_size, input_size = input_size, state_is_tuple=False)
            if training:
        ...
        self.cell = rnn_cell.MultiRNNCell(
            [create_cell(input_dim)] + [create_cell(hidden_size) for i in range(1, num_layers)], state_is_tuple=False)
