...
if __name__ == '__main__':
    ...
    parser.add_argument('--end_step', type=int, default=99)
    ...
    with tf.Graph().as_default(), tf.Session() as session:
        ...
        if config.dataset == 'softmax':
            if args.seperate:
                ...
                ### add option to start prediction from middle of music, and only evaluate for a small block. w.z. 11/24/2016
                nottingham_util.accuracy(test_probs, test_data, args.start_step, args.end_step, num_samples=args.num_samples)
